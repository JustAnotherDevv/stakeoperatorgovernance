...
export const PolkadotProvider: FC<PolkadotProviderProps> = ({
  ...
  const [activeChain, setActiveChain] =
    useState<PolkadotProviderChain>(defaultChain);
  ...
  const connect = async () => {
    ...
    try {
      ...
      const unsubscribe = await web3AccountsSubscribe((injectedAccounts) => {
        ...
        setAccount(
          !!injectedAccounts?.length ? injectedAccounts[0] : undefined
        );
      });
  ...
  const udpateSigner = async () => {
    ...
    try {
      const { web3FromSource } = await import("../wallets/extension-dapp.js");
